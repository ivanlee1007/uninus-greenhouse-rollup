...
export class UninusGreenhouseRollupCard extends HTMLElement {
  ...
  _renderFace(face){
    ...
    const controls=state.integration?`<div class="controls" aria-label="${escapeHtml(`${displayName}控制`)}">
      <button type="button" data-action="open" data-entity="${escapeHtml(state.controlEntity)}" ${directionDisabled?'disabled':''} aria-label="開啟${escapeHtml(displayName)}">▲ 開啟</button>
      <button type="button" class="stop" data-action="stop" data-entity="${escapeHtml(state.controlEntity)}" ${stopDisabled?'disabled':''} aria-label="停止${escapeHtml(displayName)}">■ 停止</button>
      <button type="button" data-action="close" data-entity="${escapeHtml(state.controlEntity)}" ${directionDisabled?'disabled':''} aria-label="關閉${escapeHtml(displayName)}">▼ 關閉</button>
    </div>`:'';
